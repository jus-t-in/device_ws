...
{
    // 使用 vector 替代可变长数组
    std::vector<uint8_t*> outbuf(this->active_camera_num, nullptr);
    uint64_t raw_timestamp;
    uint8_t camera_no = ctx[this->active_camera_num - 1].dev_node[10] - '0';

    bool res = this->mvcam->GetImagePtr(outbuf.data(), raw_timestamp, camera_no, this->g_camera_dev);
    ...
    // RCLCPP_INFO(this->get_logger(), "Camera[%d] raw timestamp = %lu", camera_no, raw_timestamp);

    // ROS 2 时间处理
    rclcpp::Time time_stamp(raw_timestamp, RCL_ROS_TIME); // 假设 raw_timestamp 单位为纳秒
    rclcpp::Time now = this->now();

    // 延迟和抖动
    rclcpp::Duration latency = now - time_stamp;
    rclcpp::Duration jitter = (this->last_time.nanoseconds() == 0) ? rclcpp::Duration(0,0) : time_stamp - this->last_time;
    this->last_time = time_stamp;

    builtin_interfaces::msg::Duration converted_latency;
    converted_latency.sec = static_cast<int32_t>(latency.seconds());
    converted_latency.nanosec = static_cast<uint32_t>(latency.nanoseconds() % 1000000000LL);

    builtin_interfaces::msg::Duration converted_jitter;
    converted_jitter.sec = static_cast<int32_t>(jitter.seconds());
    converted_jitter.nanosec = static_cast<uint32_t>(jitter.nanoseconds() % 1000000000LL);

    // 发布自定义时间消息
    miivii_gmsl_camera::msg::MiiviiTime miiviimsg;
    miiviimsg.header.stamp = time_stamp;

    for (int i = 0; i < this->active_camera_num; i++)
    {
        miiviimsg.nodes.push_back(ctx[i].dev_node);
        miiviimsg.latency.push_back(converted_latency);
        miiviimsg.jitter.push_back(converted_jitter);
    }
...
for (int i = 0; i < this->active_camera_num; i++)
{
    ...
    if (ctx[i].camera_fmt_str == "UYVY" && ctx[i].output_fmt_str == "UYVY") {
        ...
        RCLCPP_WARN(this->get_logger(), "Unsupported format for camera %d", i);
        continue;
    }

    // 用系统时间作为时间戳
    image.header.stamp = this->now();
    image.header.frame_id = "camera" + std::to_string(ctx[i].dev_node[10] - '0');
    image.height = ctx[i].out_h;
    image.width = ctx[i].out_w;
    image.encoding = encoding;
    image.is_bigendian = false;
    image.step = step_factor * image.width;
    image.data.assign(outbuf[i], outbuf[i] + image.height * image.step);
